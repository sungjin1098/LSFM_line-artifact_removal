...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230717_00_UNetv6-4', type=str)

parser.add_argument("--a", type=float, default=1e-4, help="weight for loss2 (NN, TV of S should be large)")
parser.add_argument("--b", type=float, default=1e-2, help="weight for loss3 (NN, deviation from 1 loss for S)")
...
for e in range(opt.epoch):
    ...
    for idx, img in enumerate(train_loader):
        ...
        # loss3: deviation from 1 loss for S
        # 수정해야 할 부분: 현재 코드는 mask의 상위 5%가 아니라, 마스크 전체에 deviation from 1 loss 적용 중인 상황
        loss3 = opt.b * torch.sum(5*(mask-1)**2)
    ...
    print(f'Epoch: {e + 1} / {opt.epoch}, Loss: {total_train_loss:.3f}, Loss1: {total_loss1:.3f}, Loss2: {total_loss2:.3f}, Loss3: {total_loss3:.3f}')
