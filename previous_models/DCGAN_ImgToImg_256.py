...
from DCGAN_dataloader import LineDataset_S_is_added_for_S
...
parser.add_argument("--image_size", type=int, default=256, help="default 64, training patch size")
parser.add_argument("--nz", type=int, default=400, help="default 100, latent vector size")
...
parser.add_argument("--in_dir", type=str,
                    default='./dataset/230817_LSM_simulation/230817_synthetic_S_110vol.tif',
                    help="dataset path")
...
root = f'{opt.out_dir}_DCGAN_Img2Img256_lrG_{opt.lrG}_lrD_{opt.lrD}_b{opt.batch_size}_p{opt.image_size}_nz{opt.nz}_ngf{opt.ngf}_ndf{opt.ndf}'
...
train_data = LineDataset_S_is_added_for_S(img_path=opt.in_dir, synthetic_S_path=opt.in_dir, patch_size=opt.image_size)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.ReLU(True),
            # 위의 계층을 통과한 데이터의 크기. ``(ngf*8) x 16 x 16``
            nn.ConvTranspose2d(opt.ngf * 8, opt.ngf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ngf * 4),
            nn.ReLU(True),
            # 위의 계층을 통과한 데이터의 크기. ``(ngf*4) x 32 x 32``
            ...
            nn.ReLU(True),
            # 위의 계층을 통과한 데이터의 크기. ``(ngf*2) x 64 x 64``
            nn.ConvTranspose2d(opt.ngf, opt.ngf // 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ngf // 2),
            nn.ReLU(True),
            # 위의 계층을 통과한 데이터의 크기. ``(ngf) x 128 x 128``
            nn.ConvTranspose2d(opt.ngf // 2, nc, 4, 2, 1, bias=False),
            nn.Tanh()
            # 위의 계층을 통과한 데이터의 크기. ``(nc) x 256 x 256``
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # 위의 계층을 통과한 데이터의 크기. ``(ndf*4) x 8 x 8``
            nn.Conv2d(opt.ndf * 16, opt.ndf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
...
class UNet_3Plus_for_S_64(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, feature_scale=2, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_for_S_64, self).__init__()
        ...
        self.maxpool1 = nn.MaxPool2d(kernel_size=(self.kernel, self.kernel))

        self.conv2 = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.maxpool2 = nn.MaxPool2d(kernel_size=(self.kernel, self.kernel))

        self.conv3 = unetConv2(filters[1], filters[2], self.is_batchnorm, relu=False)
        # self.maxpool3 = nn.MaxPool2d(kernel_size=(self.kernel, self.kernel))
        #
        # self.conv4 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        # self.maxpool4 = nn.MaxPool2d(kernel_size=(self.kernel, self.kernel))
        #
        ...
        #
        # self.d5tod4 = nn.Upsample(scale_factor=(self.kernel, self.kernel), mode='bilinear')
        # self.d5tod4_conv = unetConv2(filters[3], filters[2], self.is_batchnorm)
        #
        # self.d4tod3 = nn.Upsample(scale_factor=(self.kernel, self.kernel), mode='bilinear')
        self.d3_conv = unetConv2(filters[2], filters[1], self.is_batchnorm)

        self.d3tod2 = nn.Upsample(scale_factor=(self.kernel, self.kernel), mode='bilinear')
        self.d3tod2_conv = unetConv2(filters[1], filters[0], self.is_batchnorm)

        self.d2tod1 = nn.Upsample(scale_factor=(self.kernel, self.kernel), mode='bilinear')
    ...
    def forward(self, inputs):
        ...
        #
        # h5 = self.maxpool4(h4)
        hd3 = F.sigmoid(self.conv3(h3))  # h5->20*20*1024

        ## -------------Decoder-------------
        new_d3 = self.d3_conv(hd3)
        ...
        new_d1 = F.sigmoid(self.d2tod1_conv(self.d2tod1(new_d2)))

        return hd3, new_d1  # return the latent vector of line artifact mask S


class UNet_3Plus_for_S_256(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, feature_scale=2, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_for_S_256, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.is_batchnorm = is_batchnorm
        ...
        self.conv4 = unetConv2(filters[2], filters[3], self.is_batchnorm, relu=False)
        # self.maxpool4 = nn.MaxPool2d(kernel_size=(self.kernel, self.kernel))
        #
        # self.conv5 = unetConv2(filters[3], filters[4], self.is_batchnorm, relu=False)
        ...
        #
        # self.d5tod4 = nn.Upsample(scale_factor=(self.kernel, self.kernel), mode='bilinear')
        self.d4_conv = unetConv2(filters[3], filters[2], self.is_batchnorm)
    ...
    def forward(self, inputs):
        ...
        #
        # h5 = self.maxpool4(h4)
        hd4 = F.sigmoid(self.conv4(h4))  # h5->20*20*1024

        ## -------------Decoder-------------
        new_d4 = self.d4_conv(hd4)
        # new_d4 = self.d5tod4_conv(self.d5tod4(new_d5))
        new_d3 = self.d4tod3_conv(self.d4tod3(new_d4))
        new_d2 = self.d3tod2_conv(self.d3tod2(new_d3))
        new_d1 = F.sigmoid(self.d2tod1_conv(self.d2tod1(new_d2)))

        return hd4, new_d1  # return the latent vector of line artifact mask S
...
modelUNet_for_S = UNet_3Plus_for_S_256(in_channels=1, out_channels=1, feature_scale=4, is_deconv=True, is_batchnorm=True).to(device)
...
optimizer_UNetS = optim.Adam(modelUNet_for_S.parameters(), lr=opt.lrG, betas=(beta1, 0.999))
...
for epoch in range(opt.num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        _, fake = modelUNet_for_S(netG(noise))
    ...
    if (epoch + 1) % opt.valid_save_period == 0:
        with torch.no_grad():
            _, fake = modelUNet_for_S(netG(fixed_noise))
            fake = fake.detach().cpu()
            synthetic_S_grid = make_grid(real_cpu[:5].detach().cpu(), padding=2, pad_value=1)
            generated_S_grid = make_grid(fake[:5], padding=2, pad_value=1)
            save_grid = torch.cat([synthetic_S_grid, generated_S_grid], dim=1)

            save_image(save_grid, f'{root}/e{epoch + 1}.png')
            img_grid = make_grid(fake, padding=2, pad_value=1)
            save_image(img_grid, f'{root}/patches_e{epoch + 1}.png')
