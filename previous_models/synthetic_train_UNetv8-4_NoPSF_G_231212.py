...
from model_UNetv8 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator_revised
...
parser.add_argument("--out_dir", type=str, default='./231212_00', help='hyperparameters are saved at the end automatically')
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv8-4_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
for e in range(opt.epoch):
    ...
    plt.axhline(0.0077, 0, 0.1, color='blue', linestyle='solid', linewidth=2, label='PyStripe' + f', {MSE_img_gt_train:.3f}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
