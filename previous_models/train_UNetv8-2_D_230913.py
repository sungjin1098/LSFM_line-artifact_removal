...
from dataloader_3Dpre import LineDataset_S_is_added_Multiple_updates, LineDataset_valid
...
parser.add_argument("--out_dir", type=str, default='./230913_04', help='hyperparameters are saved at the end automatically')
...
parser.add_argument("--c", type=float, default=10, help="weight for loss4 (UNet_X)")
parser.add_argument("--d", type=float, default=1, help="weight for loss5 (UNet_S, Generator loss for S)")
parser.add_argument("--num_D", type=int, default=2, help="number of Discriminator learning iterations")
...
# save path
root = f'{opt.out_dir}_UNetv8-2_D{opt.num_D}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
# Dataloader
train_data = LineDataset_S_is_added_Multiple_updates(img_path=opt.in_dir, synthetic_S_path=opt.synthetic_S_dir, patch_size=opt.patch_size, num_update=opt.num_D)
