...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230525_1-2_1110', type=str)
...
parser.add_argument("--c", type=float, default=10, help="weight for loss4 (NN, S = S'''')")
...
for e in range(opt.epoch):
    ...
    for idx, img in enumerate(train_loader):
        ...
        # loss1: {input} and {output * mask_inv_STN} should be similar. --> RDN 학습에 사용
        loss1 = criterion(img, output * mask)
        # loss2: STN 거치고 mean projection 시킨 S''의 Total Variation 커야 함 --> RDN 학습에 사용
        ...
        # loss4: S와 S''''이 비슷해야 함 (S의 line이 쫙 뻗어있어야 함) --> RDN 학습에 사용
        loss4 = criterion(mask, mask_inv_STN)

        # To train RDN, SubNet1, SubNet2 --> loss1, loss2
        UNet_loss = loss1 + loss2 + loss4
