...
parser.add_argument('--frozen', type=int, default=50, help='after \'frozen\' epoch, batch normalization layers are frozen.')
...
parser.add_argument("--out_dir", type=str, default='./231030_00', help='hyperparameters are saved at the end automatically')
...
# save path
root = f'{opt.out_dir}_SupervisedSyntheticUNetv8-5_NoPSF_f{opt.frozen}_{opt.a}_{opt.c}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
for e in range(opt.epoch):
    ...
    if e >= opt.frozen:
        for moduleS in modelUNet_for_S.modules():
            if isinstance(moduleS, nn.BatchNorm2d):
                if hasattr(moduleS, 'weight'):
                    moduleS.weight.requires_grad_(False)
                if hasattr(moduleS, 'bias'):
                    moduleS.bias.requires_grad_(False)
                moduleS.eval()
        for moduleX in modelUNet_for_X.modules():
            if isinstance(moduleX, nn.BatchNorm2d):
                if hasattr(moduleX, 'weight'):
                    moduleX.weight.requires_grad_(False)
                if hasattr(moduleX, 'bias'):
                    moduleX.bias.requires_grad_(False)
                moduleX.eval()
