...
def infer(opt):
    ...
    img_3D_output = np.zeros_like(img_3D)
    mask_3D_output = np.zeros_like(img_3D)
    ...
    with torch.no_grad():
        for i in tqdm(range(img_3D.shape[0])):
            ...
            val_ind = gen_index(img_pad.size()[2:], [opt.patch+pad+pad, opt.patch+pad+pad], [opt.patch, opt.patch])
            ...
            masks = torch.zeros(img.size())

            for xi in range(len(val_ind[0])):
                for yi in range(len(val_ind[1])):
                    img_small = img_pad[:, :, val_ind[0][xi]:val_ind[0][xi] + opt.patch + pad + pad, val_ind[1][yi]:val_ind[1][yi] + opt.patch + pad + pad].to(device)

                    _, mask = modelUNet_for_S(img_small.clone().detach())
                    output = modelUNet_for_X(img_small.clone().detach())

                    if xi != len(val_ind[0]) - 1 and yi == len(val_ind[1]) - 1:
                        imgs[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:] = output[:, :, pad:-pad, pad + opt.patch - y_remain:-pad]
                        masks[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:] = mask[:, :, pad:-pad, pad + opt.patch - y_remain:-pad]
                    elif xi == len(val_ind[0]) - 1 and yi != len(val_ind[1]) - 1:
                        imgs[:, :, xi * opt.patch:, yi * opt.patch:(yi + 1) * opt.patch] = output[:, :, pad + opt.patch - x_remain:-pad, pad:-pad]
                        masks[:, :, xi * opt.patch:, yi * opt.patch:(yi + 1) * opt.patch] = mask[:, :, pad + opt.patch - x_remain:-pad, pad:-pad]
                    elif xi == len(val_ind[0]) - 1 and yi == len(val_ind[1]) - 1:
                        imgs[:, :, xi * opt.patch:, yi * opt.patch:] = output[:, :, pad + opt.patch - x_remain:-pad, pad + opt.patch - y_remain:-pad]
                        masks[:, :, xi * opt.patch:, yi * opt.patch:] = mask[:, :, pad + opt.patch - x_remain:-pad, pad + opt.patch - y_remain:-pad]
                    else:
                        imgs[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:(yi + 1) * opt.patch] = output[:, :, pad:-pad, pad:-pad]
                        masks[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:(yi + 1) * opt.patch] = mask[:, :, pad:-pad, pad:-pad]

            img_3D_output[i] = (imgs.squeeze().squeeze().detach().cpu() * high).numpy().astype('uint16')
            mask_3D_output[i] = masks.squeeze().squeeze().detach().cpu().numpy()

        io.imsave(f'{opt.out_dir}/test_output_3D.tif', img_3D_output)
        io.imsave(f'{opt.out_dir}/test_mask_3D.tif', mask_3D_output)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--patch", type=int, default=128, help='patch size, 256 means no padding')
