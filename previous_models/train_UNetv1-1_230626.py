...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230525_1-1', type=str)

parser.add_argument("--a", type=float, default=1e-2, help="weight for loss2 (NN, TV of S'' should be large)")
...
# schedulers
schedulerUNet = torch.optim.lr_scheduler.StepLR(optimUNet, step_size=50, gamma=0.5)
...
#################################################################################################################################################################################
gaussian_sigma = torch.rand(256)
param = torch.nn.parameter.Parameter(gaussian_sigma)
optimSigma = torch.optim.Adam([param], lr=1e-4)
schedulerSigma = torch.optim.lr_scheduler.StepLR(optimSigma, step_size=50, gamma=0.5)
#################################################################################################################################################################################
...
for e in range(opt.epoch):
    ...
    for idx, img in enumerate(train_loader):
        ...
        #################################################################################################################################################################################
        width = mean_proj_extend[0,0,0,:].size()
        gaussian_height = opt.patch_size
        for i in range(256):
            mean_proj_extend[0,0,:,i] = torch.Tensor(gaussian_filter1d(mean_proj_extend[0,0,:,i].detach().cpu().numpy(),param[i].detach().cpu().numpy())).to(device)
        #################################################################################################################################################################################
