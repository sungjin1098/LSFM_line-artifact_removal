...
parser.add_argument("--lrDisX", type=float, default=1e-7, help="Discriminator learning rate")
parser.add_argument("--lrDisS", type=float, default=1e-9, help="Discriminator learning rate")
...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230802_05_UNetv7-4_1_1e-2_1e-2_1e-2_lrX_1e-7_lrS_1e-9', type=str)
...
parser.add_argument("--c", type=float, default=1e-2, help="weight for loss4 (UNet_X, Generator loss for X)")
parser.add_argument("--d", type=float, default=1e-2, help="weight for loss5 (UNet_S, Generator loss for S)")
...
for e in range(opt.epoch):
    ...
    for idx, datas in enumerate(train_loader):
        ...
        synthetic_S = (1 - 0.5) * (synthetic_S - low_s) / (high_s - low_s) + 0.5
    ...
    ### Validation
    if (e + 1) % 10 == 0:
        valid_save_path = f'{temp_path}/epoch{e + 1}'
        ...
        with torch.no_grad():
            for idx, datas in enumerate(valid_loader):
                ...
                # Normalize synthetic S using max value (0~65525 -> 0~1)
                high_s = torch.max(synthetic_S)
                low_s = torch.min(synthetic_S)
                synthetic_S = (1 - 0.5) * (synthetic_S - low_s) / (high_s - low_s) + 0.5
                ...
                dis_outputX = dis_outputX.item()
                dis_outputX_rotated = dis_outputX_rotated.item()
                dis_outputS = dis_outputS.item()
                dis_outputS_synthetic = dis_outputS_synthetic.item()

                dis_outputX_his.append(dis_outputX)
                dis_outputX_rotated_his.append(dis_outputX_rotated)
                dis_outputS_his.append(dis_outputS)
                dis_outputS_synthetic_his.append(dis_outputS_synthetic)
                ###################################################

                ############# Image Save #############
                valid_idx = str(idx).zfill(2)
                mask_vis = mask[0].squeeze().detach().cpu().numpy()
                io.imsave(f'{valid_save_path}/e{e+1}_{valid_idx}_maskS_{dis_outputS:.3f}.tif', mask_vis)

                synthetic_S_vis = synthetic_S[0].squeeze().detach().cpu().numpy()
                io.imsave(f'{valid_save_path}/e{e+1}_{valid_idx}_syntheticS_{dis_outputS_synthetic:.3f}.tif', synthetic_S_vis)

                output_vis = output[0].squeeze().detach().cpu()
                output_vis = (output_vis * high).numpy().astype('uint16')  # denormalize using only max
                io.imsave(f'{valid_save_path}/e{e+1}_{valid_idx}_outputX_{dis_outputX:.3f}_r{dis_outputX_rotated:.3f}.tif', output_vis)
                ######################################
        ...
        plt.legend()
        plt.savefig(f'{valid_save_path}/e{e+1}_histogram_X.png')
        ...
        plt.ylabel('# of images')
        plt.legend()
        plt.savefig(f'{valid_save_path}/e{e + 1}_histogram_S.png')
        ###################################################
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss1_ep), label='train loss1')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Loss1')
    plt.legend(loc='upper right')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss2_ep), label='train a * loss2')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss3_ep), label='train b * loss3')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss4_ep), label='train c * loss4')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss5_ep), label='train d * loss5')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_lossDisS_ep), label='train loss_DisS')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_lossDisX_ep), label='train loss_DisX')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss of the discriminator for X')
    plt.legend(loc='upper right')
