...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230818_05_UNetv7-5_1e-1_1e-1_10_5_lrX_1e-8_lrS_1e-10_lrUX_1e-5_lrUX_1e-5', type=str)
...
for e in range(opt.epoch):
    ...
    for idx, datas in enumerate(train_loader):
        ...
        for i in range(img.size()[0]):
            top_n_value = torch.quantile(img[i].clone().detach(), n)
            top_n_gt = torch.where(img[i].clone().detach() > top_n_value, 1, 0).to(torch.float32)
