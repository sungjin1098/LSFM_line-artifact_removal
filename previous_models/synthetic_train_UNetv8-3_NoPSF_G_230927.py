...
from model_UNetv8 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator
...
parser.add_argument("--valid_save_period", type=int, default=500, help='save the valid results for every N epochs')
parser.add_argument("--model_save_period", type=int, default=500, help='save the model pth file for every N epochs')
...
parser.add_argument("--out_dir", type=str, default='./231019_02', help='hyperparameters are saved at the end automatically')

parser.add_argument("--a", type=float, default=1, help="weight for loss2 (UNet_S, TV of S should be large)")
parser.add_argument("--b", type=float, default=50, help="weight for loss3 (UNet_S, Deviation from 1 loss of mask S)")
parser.add_argument("--c", type=float, default=1, help="weight for loss4 (UNet_X)")
parser.add_argument("--d", type=float, default=50, help="weight for loss5 (UNet_S, Generator loss for S)")
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv8-3_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
modelUNet_for_S = UNet_3Plus_for_S(in_channels=1, out_channels=1, feature_scale=2, is_deconv=True, is_batchnorm=False)
...
for e in range(opt.epoch):
    ...
    for _, datas in enumerate(train_loader):
        ...
        mask_16by256, mask = modelUNet_for_S(img)
