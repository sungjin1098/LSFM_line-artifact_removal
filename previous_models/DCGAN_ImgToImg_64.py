...
parser.add_argument("--valid_save_period", type=int, default=1, help='save the valid results for every N epochs')
...
root = f'{opt.out_dir}_DCGAN_Img2Img64_lrG_{opt.lrG}_lrD_{opt.lrD}_b{opt.batch_size}_p{opt.image_size}_nz{opt.nz}_ngf{opt.ngf}_ndf{opt.ndf}'
...
modelUNet_for_S = UNet_3Plus_for_S_64(in_channels=1, out_channels=1, feature_scale=4, is_deconv=True, is_batchnorm=True).to(device)
