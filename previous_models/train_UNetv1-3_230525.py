...
from model_UNetv1 import UNet_3Plus, STN
...
parser.add_argument("--in_dir", type=str,
                    ...
                    help="dataset path")
parser.add_argument("--out_dir", default='./230525_1-3_09_11e15e65e61e7', type=str)

parser.add_argument("--a", type=float, default=1, help="weight for loss2 (NN, TV of S'' should be large)")
parser.add_argument("--b", type=float, default=1e-1, help="weight for loss3 (STN, S' = S''')")
parser.add_argument("--c", type=float, default=5e-6, help="weight for loss4 (NN, horizontal TV of S should be large)")
parser.add_argument("--d", type=float, default=5e-6, help="weight for loss5 (NN, vertical TV of S should be small)")
parser.add_argument("--e", type=float, default=1e-7, help="weight for loss6 (NN, STN of X should be small)")
...
for e in range(opt.epoch):
    ...
    for idx, img in enumerate(train_loader):
        ...
        # loss5: X의 TV는 작아야 함. --> UNet 학습에 사용
        loss6 = opt.e * tv(output)

        # To train UNet, SubNet1, SubNet2 --> loss1, loss2
        UNet_loss = loss1 + loss2 + loss4 + loss5 + loss6
    ...
    plt.clf()
    plt.plot(epoch_ep_n, np.array(train_loss6_ep), label='e * loss6')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss6, e: {str(opt.e)}')
    plt.legend(loc='upper right')
    plt.savefig(f'{plot_path}/loss6_curve.png')
    ############################################
