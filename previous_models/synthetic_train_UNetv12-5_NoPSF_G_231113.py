...
from model_UNetv12 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator, UNets, UNet_3Plus_for_X_additional
...
parser.add_argument("--lrUNetX", type=float, default=5e-4, help="UNet_3Plus learning rate")
parser.add_argument("--lrUNetS", type=float, default=5e-4, help="UNet_3Plus learning rate")
...
parser.add_argument("--in_dir", type=str, default='./dataset/231030_LSM_simulation_ExcludePSF/231030_input_Y_110vol_ExcludePSF.tif', help="dataset path")
parser.add_argument("--synthetic_S_dir", type=str, default='./dataset/231030_LSM_simulation_ExcludePSF/231030_synthetic_S_110vol_ExcludePSF.tif')
parser.add_argument("--gt_dir", type=str, default='./dataset/231030_LSM_simulation_ExcludePSF/231030_gt_110vol_ExcludePSF.tif')
parser.add_argument("--valid_idx_path", type=str, default='./dataset/231030_LSM_simulation_ExcludePSF/index_list.txt')
parser.add_argument("--out_dir", type=str, default='./231107_00', help='hyperparameters are saved at the end automatically')
...
parser.add_argument("--c", type=float, default=1, help="weight for loss4 (UNet_X)")
parser.add_argument("--d", type=float, default=10, help="weight for loss5 (UNet_S, Generator loss for S)")
parser.add_argument("--e", type=float, default=1, help="weight for loss6 (UNet_X, idea from knowledge distillation)")
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv12-5_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
modelUNet_for_X = UNet_3Plus_for_X_additional(in_channels=1, out_channels=1, feature_scale=4)
modelUNet_for_S = UNets(1, 1, bilinear=True)
...
for e in range(opt.epoch):
    ...
    for _, datas in enumerate(train_loader):
        ...
        output_1D, output = modelUNet_for_X(img)
        ...
        loss2 = -opt.a * tv(mask_1D) / ((opt.patch_size - 1) * (mask_1D.size()[1]))
        ...
        # loss6: idea from knowledge distillation
        loss6 = opt.e * criterion(mask_1D, output_1D)
        ...
        loss_UNet_for_X = loss1 + loss4 + loss6
    ...
    total_loss6 = sum(total_loss6) / len(total_loss6)
    ...
    print(f'Epoch: {e + 1} / {opt.epoch}, Total_loss: {total_train_loss:.3f}, Loss1: {total_loss1:.3f}, '
          f'Loss2: {total_loss2:.3f}, Loss3: {total_loss3:.3f}, Loss4: {total_loss4:.3f}, Loss5: {total_loss5:.3f}, Loss6: {total_loss6:.3f}')
    ...
    train_loss6_ep.append(total_loss6)
    ...
    writer.add_scalar('Loss6', total_loss6, e)
    ...
    with torch.no_grad():
        ...
        for iii, datas in enumerate(valid_loader):
            # Dataloader = (img, synthetic S)
    ...
    ax[0, 3].plot(epoch_ep_n, np.array(train_loss6_ep))
    ax[0, 3].set_title(f'{str(opt.e)} * Loss6')
    ...
    if (e + 1) % opt.model_save_period == 0:
        torch.save(modelUNet_for_S.state_dict(), f"{model_path}/UNet_for_S_{e+1}.pth")
        torch.save(modelUNet_for_X.state_dict(), f"{model_path}/UNet_for_X_{e+1}.pth")
        torch.save(modelDiscriminator_for_S.state_dict(), f"{model_path}/Discriminator_for_S_{e+1}.pth")
