...
'''
    ...
    To obtain mask S: separated encoder of UNet 3+, the latent feature is a vector + sigmoid at the last
                      decoder is just an extension of the 1D vector
...
class UNet_3Plus_for_S(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, feature_scale=4, is_deconv=True, is_batchnorm=True):
        ...
        self.maxpool3 = nn.MaxPool2d(kernel_size=(4, 1))
        ...
        self.d4tod3 = nn.Upsample(scale_factor=(4,1), mode='bilinear')
        self.d4tod3_conv = unetConv2(filters[2], filters[1], self.is_batchnorm)

        self.d3tod2 = nn.Upsample(scale_factor=(4,1), mode='bilinear')
        self.d3tod2_conv = unetConv2(filters[1], filters[0], self.is_batchnorm)

        self.d2tod1 = nn.Upsample(scale_factor=(4,1), mode='bilinear')
        self.d2tod1_conv = unetConv2(filters[0], self.in_channels, self.is_batchnorm, relu=False)
