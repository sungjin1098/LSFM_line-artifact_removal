...
parser.add_argument("--nz", type=int, default=16, help="size of the Gaussian random noise (input of GAN), shape: (nz, nz)")
parser.add_argument("--num_D", type=int, default=5, help="number of Discriminator learning iterations")
...
# save path
root = f'{opt.out_dir}_GANfromGaussian_D{opt.num_D}_{opt.c}_{opt.d}_lrDX_{opt.lrDisX}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}'
...
# Dataloader
train_data = LineDataset_S_is_added_Multiple_updates(img_path=opt.in_dir, synthetic_S_path=opt.synthetic_S_dir, gt_path=opt.gt_dir, patch_size=opt.patch_size, num_update=opt.num_D)
