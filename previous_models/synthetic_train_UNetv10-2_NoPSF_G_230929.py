...
from model_UNetv10 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator
...
parser.add_argument("--out_dir", type=str, default='./230925_00', help='hyperparameters are saved at the end automatically')
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv10-2_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
    ...
    with torch.no_grad():
        ...
        for _, datas in enumerate(valid_loader):
            ...
            if (e + 1) % opt.valid_save_period == 0:
                ...
                save_grid = torch.cat([img_grid, synthetic_S_grid, mask_grid, mask_of_X_grid, gt_grid, output_grid, color_grid], dim=1)
