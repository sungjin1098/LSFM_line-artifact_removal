...
from model_UNetv12 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator, UNets
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv12-4_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
# Define models
modelUNet_for_S = UNets(1, 1, bilinear=False)
# modelUNet_for_X = UNet_3Plus_for_X(in_channels=1, out_channels=1, feature_scale=4)
modelUNet_for_X = smp.Unet(encoder_name='mobilenet_v2', encoder_depth=5, encoder_weights='imagenet', in_channels=1,
                 decoder_use_batchnorm=True, decoder_channels=[1024, 512, 256, 128, 64])
