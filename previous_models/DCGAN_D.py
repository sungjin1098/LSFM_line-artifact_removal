...
root = f'{opt.out_dir}_DCGAN_D{opt.num_D}_lrG_{opt.lrG}_lrD_{opt.lrD}_b{opt.batch_size}_p{opt.image_size}_nz{opt.nz}_ngf{opt.ngf}_ndf{opt.ndf}'
...
train_data = LineDataset_S_is_added_Multiple_updates(img_path=opt.in_dir, synthetic_S_path=opt.in_dir,
                                                     patch_size=opt.image_size, num_update=opt.num_D)
