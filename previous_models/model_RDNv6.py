...
class RDN(nn.Module):
    def __init__(self, num_channels, num_features, growth_rate, num_blocks, num_layers):
        ...
        # separate generating S and generating X
        self.additional_layer = nn.Sequential(RDB(in_channels=1, G=4, C=2),
    ...
    def forward(self, x):
        ...
        x = self.gff2(x) + x_copy

        # 1. generate cleaned image X
        x = self.additional_layer(x)

        # 2. generate mask S
        mask = self.additional_layer(x)
