...
#### UNetv13-1 ####
# [64, 128, 64, 32, 1], bx1x1x256 --> same as UNetv8-2
###################
...
parser.add_argument("--out_dir", type=str, default='./231123_00_data1030', help='hyperparameters are saved at the end automatically')

parser.add_argument("--a", type=float, default=1, help="weight for loss2 (UNet_S, TV of S should be large)")
parser.add_argument("--b", type=float, default=10, help="weight for loss3 (UNet_S, Deviation from 1 loss of mask S)")
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv13-1_NoPSF_G{opt.num_G}_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}_b{opt.batch_size}'
...
for e in range(opt.epoch):
    ...
    for _, datas in enumerate(train_loader):
        ...
        weight = torch.clip(torch.reciprocal(mask + (1e-9 * torch.ones_like(mask))) - torch.ones_like(mask), min=0, max=10)
        loss1 = torch.mean(weight * ((img - (mask * output)) ** 2))
        loss1_for_S = criterion(img, mask * output)
    ...
    if e == 0:
        ...
        for _, datas in enumerate(valid_loader):
            ...
            MSE_img_gt_valid += MSE(img, gt)
        MSE_img_gt_valid /= len(valid_loader)
        ...
        for _, datas in enumerate(train_loader):
            ...
            MSE_img_gt_train += MSE(img, gt)
        MSE_img_gt_train /= len(train_loader)
    ...
    with torch.no_grad():
        ...
        for _, datas in enumerate(valid_loader):
            ...
            total_loss_check_valid += loss_check_valid.item()
    ...
    writer.add_scalar('valid MSE loss of {GT, output}', total_loss_check_valid, e)
    writer.add_scalar('valid MSE loss of {input, output * mask', total_loss2_check_valid, e)
    ...
    plt.axhline(MSE_img_gt_valid, 0, 0.1, color='red', linestyle='solid', linewidth=2, label='valid MSE of {GT, input}' + f', {MSE_img_gt_valid:.3f}')
    plt.axhline(MSE_img_gt_train, 0, 0.1, color='black', linestyle='solid', linewidth=2, label='train MSE of {GT, input}' + f', {MSE_img_gt_train:.3f}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
