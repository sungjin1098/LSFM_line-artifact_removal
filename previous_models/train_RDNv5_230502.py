...
from model_RDNv5 import RDN, STN
...
parser.add_argument("--out_dir", default='./230502_01_revisedloss4', type=str)
...
parser.add_argument("--c", type=float, default=1e-6, help="weight for loss4 term")
parser.add_argument("--d", type=float, default=1e-2, help="weight for loss5 term")
...
for e in range(opt.epoch):
    ...
    for idx, img in enumerate(train_loader):
        ...
        if proj_x_l1 < proj_y_l1:
            loss2 = proj_x_l1
            loss5 = torch.norm(proj_y - proj_x, 1)
        else:
            loss2 = proj_y_l1
            loss5 = torch.norm(proj_x - proj_y, 1)

        # loss1: {input} and {output * mask} must be similar.
        ...
        # loss4 = torch.sum(torch.where(mask < 1, (mask-1)**2, 5*(mask-1)**2))
        loss4 = torch.sum(torch.where(mask < 1, 5*(mask-1)**2, 0))
        loss4 = opt.c * loss4
