...
parser.add_argument("--out_dir", type=str, default='./230901_002', help='hyperparameters are saved at the end automatically')
...
parser.add_argument("--c", type=float, default=1e-1, help="weight for loss4 (UNet_X, Generator loss for X)")
parser.add_argument("--d", type=float, default=1e-1, help="weight for loss5 (UNet_S, Generator loss for S)")
...
# save path
root = f'{opt.out_dir}_SyntheticUNetv8-1_{opt.a}_{opt.b}_{opt.c}_{opt.d}_lrDX_{opt.lrDisX}_lrDS_{opt.lrDisS}_lrUX_{opt.lrUNetX}_lrUS_{opt.lrUNetS}'
...
modelUNet_for_X = UNet_3Plus_for_X(in_channels=1, out_channels=1, feature_scale=4, is_deconv=True, is_batchnorm=False)
...
for e in range(opt.epoch):
    ...
    for _, datas in enumerate(train_loader):
        ...
        dis_outputX = modelDiscriminator_for_X(output.clone())
        dis_outputX_rotated = modelDiscriminator_for_X(gt.clone())
    ...
    with torch.no_grad():
        for _, datas in enumerate(valid_loader):
            ...
            # 1. Y'' --[Enc1]-> 1D vector S --[extend]-> S'
            mask_1D, mask = modelUNet_for_S(img.clone())

            # 2. Y'' --[Enc2]-> 2D feature X --[Dec2]-> X'
            output = modelUNet_for_X(img.clone())
