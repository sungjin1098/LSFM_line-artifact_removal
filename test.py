...
from model_UNetv16 import UNet_3Plus_for_S, UNet_3Plus_for_X, STN, Discriminator_revised,UNet_for_D
...
def infer(opt):
    ...
    modelUNet_for_patch = UNet_for_D(in_channels=1, out_channels=1, feature_scale=4)

    # Data parallel
    modelUNet_for_S = nn.DataParallel(modelUNet_for_S)
    modelUNet_for_X = nn.DataParallel(modelUNet_for_X)
    modelUNet_for_patch = nn.DataParallel(modelUNet_for_patch)
    ...
    modelUNet_for_patch.load_state_dict(torch.load(opt.modelUNet_for_dis_patch_path))

    # to device
    modelUNet_for_S = modelUNet_for_S.to(device)
    modelUNet_for_X = modelUNet_for_X.to(device)
    modelUNet_for_patch = modelUNet_for_patch.to(device)

    # eval mode
    modelUNet_for_S.eval()
    modelUNet_for_X.eval()
    modelUNet_for_patch.eval()
    ...
    patch_3D_output = torch.zeros_like(torch.Tensor(img_3D.astype('float32')))
    ...
    with torch.no_grad():
        for i in tqdm(range(img_3D.shape[0])):
            ...
            patchs = torch.zeros(img.size(), dtype=torch.float32)
            for xi in range(len(val_ind[0])):
                for yi in range(len(val_ind[1])):
                    ...
                    if xi != len(val_ind[0]) - 1 and yi == len(val_ind[1]) - 1:
                        ...
                        patchs[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:] = patch[:, :, opt.pad:-opt.pad, opt.pad + opt.patch - y_remain:-opt.pad]
                        ...
                        patchs[:, :, xi * opt.patch:, yi * opt.patch:(yi + 1) * opt.patch] = patch[:, :, opt.pad + opt.patch - x_remain:-opt.pad, opt.pad:-opt.pad]
                        ...
                        patchs[:, :, xi * opt.patch:, yi * opt.patch:] = patch[:, :, opt.pad + opt.patch - x_remain:-opt.pad, opt.pad + opt.patch - y_remain:-opt.pad]
                        ...
                        patchs[:, :, xi * opt.patch:(xi + 1) * opt.patch, yi * opt.patch:(yi + 1) * opt.patch] = patch[:, :, opt.pad:-opt.pad, opt.pad:-opt.pad]


            img_3D_output[i] = (imgs.squeeze().squeeze().detach().cpu() * high)
            mask_3D_output[i] = masks.squeeze().squeeze().detach().cpu()
            patch_3D_output[i] = (patchs.squeeze().squeeze().detach().cpu())

        io.imsave(f'{opt.out_dir}/test_1205.tif', img_3D_output.numpy().astype('uint16'))
        io.imsave(f'{opt.out_dir}/test_1205_mask.tif', mask_3D_output.numpy())
        io.imsave(f'{opt.out_dir}/test_stripe_real.tif', patch_3D_output.numpy().astype('uint16'))
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--in_dir', type=str, default='./dataset/stitch_exp.tif')

    
    #parser.add_argument('--in_dir', type=str, default='./dataset/231030_LSM_simulation_ExcludePSF/1030_re.tif')


    #250이 가장 좋음
    #
    #1460 1170 1160 210 220 380
    #parser.add_argument('--modelUNet_for_S_path', type=str, default='./UNet_for_S_1125_2.pth')
    #parser.add_argument('--modelUNet_for_X_path', type=str, default='./UNet_for_X_1126.pth')


    parser.add_argument('--modelUNet_for_S_path', type=str, default='./UNet_for_S_1520.pth')
    parser.add_argument('--modelUNet_for_X_path', type=str, default='./UNet_for_X_1520.pth')

    parser.add_argument('--modelUNet_for_dis_patch_path', type=str, default='./Discriminator_for_patch_100.pth')
    parser.add_argument('--out_dir', type=str, default='./test_output')
    parser.add_argument("--patch", type=int, default=256, help='patch size, 256 is default')
    parser.add_argument("--pad", type=int, default=512, help='padding, 512 is default')
